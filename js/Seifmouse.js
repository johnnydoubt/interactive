...
window.onload = function(){  //chargement de la page
	var canvas = document.createElement("canvas"); // Récupération du canvas
	var ctx = canvas.getContext("2d"); // indiquer le contexte, ici 2D.
	...
	var effects = []; // création d'un tableau d'effets
	var mouse = {}; // création d'un objet mouse

	// On commence par créer le nombre d'effets
	var effect_count = 80;
	for(var i = 0; i < effect_count; i++) // incrémentation du nombre d'effets
	{
	...
	function draw()
	{
		...
		for(var i = 0; i < effects.length; i++){ // incrémentation des effets.
			...
			var gradient = ctx.createRadialGradient(e.location.x, e.location.y, 0, e.location.x, e.location.y, e.radius); // On crée un objet, un dégradé radial avec ses arguments, ici  tout les arguments sont assignés a la position de la souris.
